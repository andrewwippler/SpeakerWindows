...
export default class SettingsController {
  public async index({ auth }: HttpContext) {
    ...
    var settings = await Setting.query()
    ...
    const tag = await Illustration.query().whereHas('tags', (query) => {
    ...
    const tagQuery2 = await Illustration.query().where('user_id', `${auth.user?.id}`).doesntHave('tags')
