...
test.group('Place', (group) => {
  ...
  group.setup(async () => {
    ...
    const place1 = await PlaceFactory.merge({
      illustration_id: illustration.id,
      user_id: goodUser.id,
    }).create()
    const place2 = await PlaceFactory.merge({
      illustration_id: illustration.id,
      user_id: goodUser.id,
    }).create()
    const place3 = await PlaceFactory.merge({
      illustration_id: illustration.id,
      user_id: goodUser.id,
    }).create()
    const place4 = await PlaceFactory.merge({
      illustration_id: illustration.id,
      user_id: goodUser.id,
    }).create()
    const place5 = await PlaceFactory.merge({
      illustration_id: illustration.id,
      user_id: goodUser.id,
    }).create()
  })
  ...
  test('Fail on creating to nonexistent illustration', async ({ client, assert }) => {
    ...
    const response = await client
      .post(`/places/999999999999999999999999999999999`)
      .bearerToken(loggedInUser.body().token)
      .json(place.toJSON())
