...
test.group('Illustration model search', (group) => {
  ...
  test('lexical only match and semantic only match and fuzzy match and mixed', async ({
    assert,
  }) => {
    const user = await UserFactory.create()

    // Create illustrations with titles designed for different retrieval methods
    const lex = await IllustrationFactory.merge({
      title: 'Lexical Match',
      user_id: user.id,
    }).create()
    const fuzzy = await IllustrationFactory.merge({
      title: 'Fuzzyy Mismatch',
      user_id: user.id,
    }).create()
    const sem = await IllustrationFactory.merge({
      title: 'Semantic meaning',
      user_id: user.id,
      content: 'deep learning and vectors',
    }).create()
